...
DUCKDNS_DOMAIN="jherrerog-wordpress.duckdns.org"  # CAMBIAR POR DOMINIO DE WORDPRESS
...
if [ -f "$CERT_PATH" ]; then
    ...
    sudo certbot certonly --standalone -d $DUCKDNS_DOMAIN --non-interactive --agree-tos --email jherrerog03@educantabria.es
fi
